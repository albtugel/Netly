...
Current state: Profile Service has a skeleton with a `/health` endpoint. Budget Service serves a JWT-protected CRUD API for subscriptions, debts and savings goals, with monthly equivalents computed on read and a unified `application/problem+json` error format (see [`services/budget-service/README.md`](services/budget-service/README.md)). Spending Service is designed; its implementation is planned.
